...
// Batch queue for database operations
const dbBatchQueue = new Map();
let batchTimer = null;
const BATCH_INTERVAL_MS = 500; // Process batches every 500ms

async function recoverActiveSessions() {
const pool = getPool();

try {
const result = await pool.query(`SELECT  ds.device_key, ds.frontend_id, ds.device_name, ds.is_running, ds.session_started_at, ds.current_mode, ds.current_equipment_status, ds.last_temperature, ds.last_heat_setpoint, ds.last_cool_setpoint, rs.session_id FROM device_status ds LEFT JOIN runtime_sessions rs ON ds.device_key = rs.device_key AND rs.ended_at IS NULL WHERE ds.is_running = true`);
...
if (delay > 60) {
  console.log(`WARNING: Event delayed by ${delay} seconds (${Math.floor(delay/60)} minutes)`);
}
...
if (traits['sdm.devices.traits.Connectivity']) {
  ...
  operations.push(updateDeviceReachability(deviceKey, isReachable, eventTimestamp));
}

// Handle temperature (non-blocking)
if (traits['sdm.devices.traits.Temperature']) {
  ...
  handleTemperatureChange(deviceKey, tempF, tempC, userId, eventTimestamp).catch(err => {
...
console.log(`========== EVENT PROCESSING COMPLETE (${elapsed}ms) ==========\n`);
```
...
const elapsed = Math.floor((eventTimestamp - currentState.sessionStartedAt) / 1000);
...
// Log equipment event asynchronously - don’t await
logEquipmentEvent(deviceKey, equipmentStatus, isFanTimerOn, currentState, eventTimestamp).catch(err => {
console.error(‘Error logging equipment event (non-blocking):’, err);
});
}
...
console.log(`  Fan Timer: ${isFanTimerOn ? 'ON' : 'OFF'}`);
console.log(`  Event Time: ${eventTimestamp.toISOString()}`);
...
console.log(`Session started at ${eventTimestamp.toISOString()}`);
console.log(`isHvacActive is now: TRUE`);

// POST TO BUBBLE - NON-BLOCKING
console.log('\n>>> POSTING SESSION START TO BUBBLE (ASYNC) <<<');
...
console.log(`                  Ended: ${eventTimestamp.toISOString()}`);
...
if (totalRuntimeSeconds > 0) {
  console.log('\n>>> POSTING RUNTIME EVENT TO BUBBLE (ASYNC) <<<');
  postToBubbleAsync({
    userId: userId,
    thermostatId: deviceKey,
    deviceName: deviceName,
...
const elapsed = Math.floor((eventTimestamp - deviceState.sessionStartedAt) / 1000);
...
// Execute updates in parallel
await Promise.all([
pool.query(`UPDATE runtime_sessions SET tick_count = tick_count + 1, last_tick_at = $2, heat_setpoint = $3, cool_setpoint = $4, updated_at = NOW() WHERE session_id = $1`, [deviceState.sessionId, eventTimestamp, heatSetpoint, coolSetpoint]),
...
const [updateResult, deviceResult] = await Promise.all([
  pool.query(`
    UPDATE device_status SET
      last_temperature = $2,
      last_seen_at = $3,
...
// Insert temp reading asynchronously - don't block
pool.query(`
...
`, [deviceKey, tempF, sessionId, eventTimestamp]).catch(err => {
  console.error('Error inserting temp reading (non-blocking):', err);
});
...
console.log('\n>>> POSTING TEMPERATURE EVENT TO BUBBLE (ASYNC) <<<');
...
try {
await pool.query(`UPDATE device_status SET is_reachable = $2, last_seen_at = $3, updated_at = NOW() WHERE device_key = $1`, [deviceKey, isReachable, eventTimestamp]);
...
try {
await pool.query(`INSERT INTO equipment_events ( device_key, event_type, equipment_status, previous_status, is_active, session_id, event_data, recorded_at ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
deviceKey,
‘status_change’,
equipmentStatus,
